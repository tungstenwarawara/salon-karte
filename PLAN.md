...
| パターン | log_type | quantity | トリガー |
|---------|----------|----------|---------|
| 仕入れ | `purchase_in` | +N | 手入力 |
| 店頭販売 | `sale_out` | -N | 物販登録で自動 |
| EC販売 | `ec_out` | -N | EC注文で自動（将来） |
| サンプル使用 | `sample_out` | -N | 手入力（理由メモ付き） |
| 破損・廃棄 | `waste_out` | -N | 手入力（理由メモ付き） |
...
```
1. 顧客選択 → 2. 商品選択（売価自動セット）→ 3. 数量入力 → 4. 記録

裏側: ① purchases INSERT → ② inventory_logs INSERT (sale_out) → ③ 在庫更新
      ④ 在庫≤発注点 → ダッシュボードにアラート
```
