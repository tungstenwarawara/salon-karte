  ...
  return (
    <div className="bg-surface border border-border rounded-2xl p-4 space-y-3">
        ...
        <div key={s.id} className="border border-border rounded-lg p-3 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs text-accent hover:underline min-h-[32px] px-2"
            >
        ...
        <div key={s.id} className="border border-border rounded-lg p-3 space-y-2">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 text-xs border border-border rounded-lg py-2 min-h-[36px] hover:bg-background transition-colors"
            >
              {copied ? "コピーしました" : "URLをコピー"}
            </button>
            <button
              onClick={() => handleShowQr(s.token)}
              className="text-xs border border-border rounded-lg px-3 py-2 min-h-[36px] hover:bg-background transition-colors"
            >
              QR
            </button>
          </div>
      ...
      <button
        ...
        disabled={creating}
        className="w-full text-sm text-accent border border-accent/30 rounded-lg py-2.5 min-h-[44px] hover:bg-accent/5 transition-colors disabled:opacity-50"
      >
        {creating ? "発行中..." : "カウンセリングシートを発行"}
      </button>
    </div>
  );
}
