...
export function WeekViewContainer({ salonId, selectedDate, businessHours, salonHolidays }: Props) {
  ...
  // スケジュール解決（useMemoで再計算を防止）
  const overridesForCalc: StaffScheduleOverride[] = useMemo(() => overrides.map((o) => ({
    staff_id: o.staff_id, override_date: o.override_date,
    is_working: o.is_working, start_time: o.start_time, end_time: o.end_time,
  })), [overrides]);
  const weeklySchedules = useMemo(() => getStaffSchedulesForWeek(weekStart, staffList, businessHours, salonHolidays, overridesForCalc), [weekStartStr, staffList, businessHours, salonHolidays, overridesForCalc]); // eslint-disable-line react-hooks/exhaustive-deps

  // 予約を日付でグループ化
  const appointmentsByDate = useMemo(() => {
    const map = new Map<string, FetchedAppointment[]>();
    for (const a of appointments) {
      const list = map.get(a.appointment_date);
      if (list) list.push(a); else map.set(a.appointment_date, [a]);
    }
    return map;
  }, [appointments]);
  ...
  const columns: ColumnDef[] = useMemo(() => {
    ...
    if (!isMultiStaff) {
      ...
      const dayAppts = appointmentsByDate.get(selectedDateStr) ?? [];
      return staffList.map((staff) => {
        ...
        return {
          ...
          appointments: dayAppts.filter((a) => a.staff_id === staff.id),
        };
      });
    }
  }, [staffList, isMultiStaff, weeklySchedules, dates, selectedDayIndex, appointmentsByDate]);
  ...
  const { startHour, endHour } = useMemo(() => {
    ...
    for (const col of columns) {
      if (col.schedule.isWorking) {
        min = Math.min(min, Math.floor(timeToMinutes(col.schedule.startTime) / 60));
        max = Math.max(max, Math.ceil(timeToMinutes(col.schedule.endTime) / 60));
      }
    }
  ...
  // 曜日タブ用の予約件数
  const appointmentCounts = useMemo(() => dates.map((dateStr) =>
    (appointmentsByDate.get(dateStr) ?? []).length
  ), [dates, appointmentsByDate]);
  ...
  if (staffList.length === 0) {
    return (
      <div className="bg-surface border border-border rounded-xl p-6 text-center">
        ...
        <Link href="/settings/staff" className="inline-block mt-2 text-sm text-accent hover:underline font-medium">
          スタッフを登録する →
        </Link>
      </div>
    );
