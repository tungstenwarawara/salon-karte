...
import { todayStrInJst, jstDateStringAfterDays } from "@/lib/business-hours";
...
export async function GET(
  ...
  // 日付バリデーション（JST基準。booking-slots.ts の判定と基準を揃える）
  const todayStr = todayStrInJst();
  ...
  // 60日先まで制限
  const maxDateStr = jstDateStringAfterDays(60);
  if (date > maxDateStr) {
