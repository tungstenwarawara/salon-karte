...
#### 商用リリース前の必須対応
- **独自ドメイン認証（SPF/DKIM/DMARC）**: 未設定だとメールが迷惑メールフォルダに入る確率が高い
  - Resend ダッシュボード → Domains でDNSレコードを設定
  - SPF: `v=spf1 include:amazonses.com ~all`（Resend は Amazon SES 経由）
  - DKIM: Resend が提供する CNAME レコードを DNS に追加
  - DMARC: `v=DMARC1; p=none; rua=mailto:dmarc@salonkarte.com`
  - **Phase A で DNS 設定と同時に実施**
...
- Push API で送信するメッセージ（予約確認・リマインド）は通数にカウントされる
- 1サロンあたり月50-100通 → 無料枠で十分
- salon-karte のインフラコストには影響なし

#### 商用リリース前の必須対応
- **サロンオーナー向けにLINEプラン制限の説明が必要**
  - LINE設定画面（`/settings/line`）に月200通制限の注意書きを表示
  - 超過した場合の挙動（API エラー 429）のハンドリング
  - 有料プラン（ライトプラン: 月5,000通/5,000円）へのアップグレード案内
...
#### 商用リリース前の必須対応
- **プライバシーポリシーへのGA4/Cookie使用の明記**
  - `/privacy` ページにGA4によるアクセス解析の記載を追加
  - 収集データ: ページビュー、セッション時間、参照元等（個人を特定しない匿名データ）
  - Cookie の種類と有効期限の記載
- **Cookie同意バナー（Phase B-4 で対応予定）**
  - 同意前は GA4 スクリプトを読み込まない
  - LP のみ表示（ダッシュボードはログイン済みユーザーのみ）
