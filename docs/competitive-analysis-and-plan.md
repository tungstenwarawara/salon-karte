...
> 更新日: 2026-03-11（v8 — SEO記事5本公開、キャリアメール対応追加）
...
| | 複数メニュー + 支払方法（現金/クレジット/回数券/サービス） | ✅ 実装済み |
| | カルテ内で物販・回数券購入を統合 | ✅ 実装済み |
| | 予約からのメニュー自動引き継ぎ | ✅ 実装済み |
...
| | 現金/クレジット内訳・回数券消化額・サービス額 | ✅ 実装済み |
| | 前受金（未消化分合計）表示 | ✅ 実装済み |
| | 確定申告レポート + CSV出力 | ✅ 実装済み |
| | 顧客単位の売上サマリー | ✅ 実装済み |
| 売上分析 | 新規/リピーター推移グラフ | ✅ 実装済み |
| | 顧客LTVサマリー | ✅ 実装済み |
| 物販管理 | 購入記録の登録・一覧 | ✅ 実装済み |
| 回数券管理 | チケット登録・消化管理・有効期限 | ✅ 実装済み |
| | 手動調整（残回数の修正） | ✅ 実装済み |
| 在庫管理 | 商品マスタ CRUD | ✅ 実装済み |
| | 在庫一覧・発注点アラート | ✅ 実装済み |
| | 仕入（入庫）・サンプル/廃棄（出庫） | ✅ 実装済み |
| | 物販登録 → 自動出庫連動 | ✅ 実装済み |
| | 棚卸し機能 | ✅ 実装済み |
| | 商品CSVインポート | ✅ 実装済み |
| LINE連携 | Webhook（友だち追加/ブロック検知） | ✅ 実装済み |
| | 顧客とLINE友だちの紐付け | ✅ 実装済み |
| | 予約確認通知（LINE送信） | ✅ 実装済み |
| | 前日リマインド（Vercel Cron 21:00 JST） | ✅ 実装済み |
| | 既存フォロワー同期 | ✅ 実装済み |
| | セットアップガイド（3ステップ） | ✅ 実装済み |
| カウンセリング | テンプレート作成（ドラッグ&ドロップ並べ替え） | ✅ 実装済み |
| | QRコード/URL発行（顧客が自分のスマホで回答） | ✅ 実装済み |
| | 回答自動保存・顧客紐付け | ✅ 実装済み |
...
| | 料金プラン管理画面 | ✅ 実装済み |
| LP | ランディングページ（11セクション + プログラマティックモック + コストシミュレーター） | ✅ 実装済み |
...
| | 使い方ガイド | ✅ 実装済み |
| | 特定日の営業時間変更 | ✅ 実装済み |
| 設定UX | 設定ページのカテゴリ分け（6カテゴリ） | ✅ 実装済み |
| | HelpTip吹き出し（複雑な設定項目に?アイコン） | ✅ 実装済み |
| | LINE設定の用語改善（専門用語→平易な日本語） | ✅ 実装済み |
| | LINE設定FAQ（よくあるトラブル対応） | ✅ 実装済み |
| | 顧客詳細ページからのLINE紐付け（双方向導線） | ✅ 実装済み |
...
- `/sales/inventory/products` - 商品マスタ
- `/sales/inventory/receive` - 仕入（入庫）
...
| **salon-karte** | **2,980円** | **0円** | **○無制限** | **○** | **○** | **○（込み）** | **○（込み）** | **計画中** | **○（込み）** | **エステ** |
...
| 来店分析 | 来店回数・間隔・最終来店日 |
| LINE連携（予約確認+リマインド） | **KaruteKun +5,500円 → 込みで圧倒的優位** |
| カウンセリングシート（QRコード発行） | **coming-soonと同等。込みで差別化** |
...
#### G. 設定ページUX改善 ✅ 実装済み（2026-03-06）

テスターから「LINE顧客紐付けができない」との問い合わせを受けて実施。

| 施策 | 内容 | 状態 |
|------|------|------|
| 設定カテゴリ分け | 12項目のフラットリスト→6カテゴリ（基本設定/予約・メニュー/連携・通知/スタッフ/データ管理/アカウント） | ✅ |
| LINE紐付け双方向導線 | 顧客詳細ページからもLINE紐付け可能に + 検索機能付き | ✅ |
| LINE用語改善 | 「チャネルID」→「接続番号」等、専門用語を排除（括弧で元用語も併記） | ✅ |
| LINE FAQ追加 | 通知が届かない/紐付け方法/接続番号の場所をaccordionで追加 | ✅ |
| HelpTipコンポーネント | ?アイコンタップで説明吹き出し表示。LINE設定・予約受付設定・Web予約設定に配置 | ✅ |
...
### Phase 6.0: カルテ追加機能 ✅ 完了

| # | 施策 | 状態 |
|---|------|------|
| 6-1 | カルテ複数メニュー + 支払方法 + 予約引き継ぎ | ✅ 実装済み |
| 6-2 | カルテへの物販・回数券購入の統合 | ✅ 実装済み |
| 6-3 | 売上集計の修正 + 経営指標（前受金・現金/クレジット内訳） | ✅ 実装済み |
| 6-4 | カウンセリングシート（テンプレート + QRコード + 回答管理） | ✅ 実装済み |
| 6-5 | LINE連携（Webhook + 予約確認 + リマインド + 友だち管理） | ✅ 実装済み |
...
| # | 施策 | 状態 |
|---|------|------|
| — | LP（11セクション + JSON-LD + OGP + アニメーション + コストシミュレーター） | ✅ 実装済み |
...
### 未実装（Phase 2 商用リリース準備で対応）

| # | タスク | 優先度 | 備考 |
|---|--------|--------|------|
| 1 | **各ページでの機能ゲート適用** | 必須 | `plan.ts` 定義済み → 各ページで制限チェック |
| 2 | ~~データポータビリティの明記~~ | ~~必須~~ | ✅ 利用規約 第6条の2 + LP FAQ に追記済み |
| 3 | ~~新規登録時の初期プラン = free~~ | ~~必須~~ | ✅ setup で plan_type: "free" を明示設定済み |
| 4 | **Rate Limiting（公開API）** | 必須 | |
| 5 | ~~Stripe Webhook idempotency~~ | ~~必須~~ | ✅ stripe_processed_events テーブルで冪等性保証 |

### 実装済み（Phase 0 完了分）

| タスク | 状態 |
|--------|------|
| SEOブログ機能（`/blog` + 記事2本） | ✅ 実装済み |
| 紹介特典システム（紹介コード + 双方1ヶ月無料） | ✅ 実装済み |
| 利用規約にサービス終了条項（第10条） | ✅ 実装済み |
| LP プログラマティックモック（実アプリUI忠実再現） | ✅ 実装済み |
| Cookie 同意バナー | ✅ 実装済み |
| 会計ソフト連携CSV出力（freee/マネーフォワード/弥生） | ✅ 実装済み |
...
### A. salon-karte（カルテ管理 — ✅ 全Phase完了）

**対象**: コア機能 + UI/UX改善 + 在庫管理 + 確定申告 + カルテ追加機能

| 項目 | 内容 |
|------|------|
| Phase 1-3 | コア機能（顧客・カルテ・予約・物販・回数券） ✅ |
| Phase 4 | UI/UX改善（FAB・パンくず・オンボーディング） ✅ |
| Phase 5 | 在庫管理 + 確定申告サポート ✅ |
| Phase 6 | LINE連携・カウンセリング・売上分析・CSV・カルテ強化 ✅ |
| 実績 | **約3週間で全6フェーズ完了** |
| 月額収益 | 2,980円/サロン（全機能込み） |
