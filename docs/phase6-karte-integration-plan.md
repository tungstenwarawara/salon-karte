# カルテ・予約・物販の動線再整理 — Phase 6 実装計画

> 作成日: 2026-02-20
> 最終更新: 2026-02-22（全Phase実装完了を反映）
> ステータス: **✅ 全Phase実装完了**
...
## Phase 6-1: カルテ複数メニュー化 + 支払方法 + 予約引き継ぎ ✅ 完了
...
全Phase実装完了（2026-02-22）:

```
Phase 6-1 ✅: カルテ複数メニュー + 支払方法 + 回数券消化選択
Phase 6-2 ✅: カルテ画面に物販・回数券購入を統合
Phase 6-3 ✅: 売上集計修正（前受金・現金/クレジット内訳・参考指標追加）
Phase 6-4 ✅: 顧客詳細の表示改善

追加実装:
  - カウンセリングシート（テンプレート + QRコード + 顧客回答）
  - LINE連携（Webhook + 予約確認 + リマインド + 友だち管理）
  - CSVエクスポート/インポート（顧客・カルテ・商品）
  - 売上分析（新規/リピーター推移・LTVサマリー）
```
...
### 汎用性・シンプルさ・パフォーマンスの3原則

テスターの要望を満たしつつ、回数券を使わないサロン（美容室・ネイル等）にも自然なUIを提供するために、以下の3原則を全Phase共通で適用する。

#### 1. 段階的開示（Progressive Disclosure）

回数券関連UIの複雑さは、データの有無に応じて動的に表示・非表示を切り替える。

| 状態 | 表示 |
|------|------|
| 顧客が回数券を持っていない | 支払方法セクション自体を非表示。メニュー選択のみのシンプルなUI |
| 回数券が1つだけ | 「回数券から消化」トグルのみ。選択UIは省略して自動選択 |
| 回数券が複数 | フルUI表示（どのチケットから消化するか選択） |
| 物販・回数券購入セクション | デフォルト折りたたみ。使う人だけ展開 |

**効果**: 回数券を使わないサロンのオーナーには、従来とほぼ同じシンプルなカルテ作成画面が表示される。

#### 2. 条件付きフェッチ（Conditional Fetch）

不要なクエリを実行しないことで、パフォーマンスを最適化する。ダッシュボードの在庫アラート（商品登録時のみ表示）と同じパターン。

| 画面 | 現在 | Phase 6後 | 最適化 |
|------|------|----------|--------|
| カルテ作成 | メニュー一覧1クエリ | + 回数券一覧 + 予約メニュー | 顧客選択後に回数券の有無を`head: true`で確認。ある場合だけ詳細取得（+1〜2） |
| カルテ詳細 | 1クエリ | + treatment_record_menus + 紐づく物販/回数券 | `Promise.all`で並列化。体感は変わらない（+2〜3並列） |
| 顧客詳細 | 5〜6クエリ | + treatment_record_menusのJOIN | JOINで吸収。クエリ数増加なし（+0） |
| ダッシュボード | 9+2クエリ | 変更なし | 影響なし（+0） |

**効果**: 回数券を使わないサロンでは追加クエリがゼロ。ダッシュボードは全く影響なし。

#### 3. 折りたたみデフォルト（Collapsed by Default）

カルテ作成画面が巨大化しないよう、追加セクションは閉じた状態で開始する。

```
━━ 施術メニュー ━━          ← 常に表示（メイン機能）
━━ 施術記録 ━━              ← 常に表示（メイン機能）
▶ 回数券購入（任意）          ← 折りたたみ。タップで展開
▶ 物販記録（任意）            ← 折りたたみ。タップで展開
```

- 既存の `CollapsibleSection` コンポーネントを再利用
- 初めて使うサロンオーナーには上2つだけが見え、従来と同じシンプルさ
- 回数券販売や物販が必要な場合だけ展開して入力
