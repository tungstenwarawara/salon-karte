...
スタンダードプラン2,980円に含まれる差別化機能:

| 機能 | 競合状況 | salon-karte | 2,980円に含める根拠 |
|------|---------|------------|-------------------|
...
### LINE連携の実装内容（2026-02-22完了）

| 機能 | 内容 |
|------|------|
| Webhook | 友だち追加/ブロック検知 → customer_line_links テーブルに自動記録 |
| 予約確認通知 | 予約作成時にLINE Push Message（fire-and-forget） |
| 前日リマインド | Vercel Cron（21:00 JST） → 翌日の予約者にリマインド |
| 友だち管理 | LINE友だち一覧 → 顧客との紐付けUI（検索・ソート・件数表示） |
| 既存フォロワー同期 | GET /v2/bot/followers/ids（認証済みアカウント以上） |
| セットアップガイド | 3ステップガイド（LINE Official Account Manager → Developers Console） |
